...
from .grid_track import GridTrack, TILE_MURO, TILE_PAVIMENTO
...
class RacingEnv(gym.Env):
    """Entorno de carrera con observación tipo patch egocéntrico (para DQN).

    - El coche avanza hacia +X, puede desplazarse lateralmente (y) y acelerar/frenar.
    - Únicamente choca contra muros (tile=1).
    - Superficies modifican la velocidad (aceite/terracería/boost).
    - Meta: llegar cerca del borde derecho (x >= ancho - 2).
    """
    metadata = {"render_modes": ["human", "rgb_array"]}

    def __init__(self, ruta_csv: str, patch_h: int = 11, patch_w: int = 11, render_mode: str | None = None, renderer_ppu: int = 36, render_fps: int = 60):
        super().__init__()
        ...
        # Centro ideal (fila media de carril)
        ...
        # Espacios Gym
        # Observación: one-hot de 6 canales (0..5) con tamaño patch_h × patch_w
        #self.observation_space = spaces.Box(low=0.0, high=1.0, shape=(6, self.patch_h, self.patch_w), dtype=np.float32) 

        self.observation_space = spaces.Box(
            low=0.0, high=1.0,
            shape=(self.patch_h, self.patch_w, 6),  # H, W, C
            dtype=np.float32
        )
     
        # Acción: (steer × throttle) = 3×3 = 9 combinaciones
    ...
    def _accion_a_tuplas(self, a: int) -> tuple[int, int]:
        # mapeo 0..8 -> (steer_idx, throttle_idx)
    ...
    def reset(self, seed: int | None = None, options: dict | None = None):
        ...
        # Posición inicial (x pequeña, sobre pavimento, centrado en Y)
        self.x = 2.0
        self.y = self.y_centro
        self.v = 0.0
        self._x_prev = self.x
        # Observación inicial
    ...
    def step(self, action: int):
        ...
        # Actualizamos dinámica
        x_new, y_new, v_new = self.dyn.actualizar(self.x, self.y, self.v, (steer_idx, throttle_idx), tile_bajo)

        # Limitamos Y para no salir del grid vertical (afueras no colisionan pero limitamos por simplicidad)
        y_new = float(np.clip(y_new, 0.0 + self.CAR_ALTO_Y/2.0, self.track.alto - self.CAR_ALTO_Y/2.0))

        # Comprobamos colisión con muro con el rectángulo del coche
        ...
        # Meta: llegar al final del ancho
        llego_meta = (x_new >= (self.track.ancho - 2))
        ...
        truncated = False  # se puede agregar límite de pasos externo
    ...
    def set_visual_speed_scale(self, escala: float):
        """Escala el tiempo/velocidad SOLO para visualización."""
    ...
    def render(self):
        if self.renderer is None:
            return
        self.renderer.draw(self.track.grid, self.x, self.y, self.CAR_LARGO_X, self.CAR_ALTO_Y, fps=self.render_fps)
